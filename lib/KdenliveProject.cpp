...
// KdenliveProject --------------------------------------------------
// CONSTRUCTORS
KdenliveProject::KdenliveProject(const float framerate, const int frame_width, const int frame_length){
...
void KdenliveProject::GenerateFile(const vector<string> &media_folder_paths){ // Kind of a mess, needs reworking
	// Reset the file
	kdenlive_file.~KdenliveFile();	// Destructor doesn't do anything currently
	new (&kdenlive_file) KdenliveFile();
	
	// Start the document
	kdenlive_file.SetProfile(framerate, frame_width, frame_length);
...
string KdenliveProject::SaveAsString(const vector<string> &media_folder_paths){
	// Generate the file
	GenerateFile(media_folder_paths);

	// Return file string
	return kdenlive_file.ToString();
}

void KdenliveProject::SaveToFile(const vector<string> &media_folder_paths, const string &output_filepath, const string &file_name){
	// Generate the file
	GenerateFile(media_folder_paths);
	
	kdenlive_file.SaveToFile(output_filepath, file_name);
}
